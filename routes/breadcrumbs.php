...
Breadcrumbs::for('admin.settings.smm', function (BreadcrumbsTrait $trail) {
    $trail->parent('admin.settings');
    $trail->push('Cấu hình SMM API');
});

Breadcrumbs::for('deposit.bank', function (BreadcrumbsTrait $trail) {
    $trail->parent('deposit');
    $trail->push('Chuyển khoản ngân hàng');
});
...
Breadcrumbs::for('api.docs.index', function (BreadcrumbsTrait $trail) {
    $trail->parent('home');
    $trail->push('T�i li?u API', route('api.docs.index'));
});

Breadcrumbs::for('admin.settings.levels', function (BreadcrumbsTrait $trail) {
    $trail->parent('admin.settings');
    $trail->push('C?u h�nh C?p d? & Th�ng b�o');
});

Breadcrumbs::for('admin.queue.index', function ($trail) { $trail->parent('home'); $trail->push('Qu?n l� Worker', route('admin.queue.index')); });

Breadcrumbs::for('admin.orders.cancelled', function ($trail) { $trail->parent('home'); $trail->push('�on d� h?y', route('admin.orders.cancelled')); });
