...
@section('content')
    <div class="row">
        ...
        <div class="col-lg-6">
            <div class="card">
                ...
                <div class="card-body">
                    <form action="{{ route('admin.settings.store') }}" method="post">
                        @csrf
                        <div class="mb-3">
                            <label for="charge_note">Ghi chú nạp tiền</label>
            ...
            <div class="card">
                <div class="card-header align-items-center d-flex">
                    <h4 class="card-title mb-0 flex-grow-1">Thông tin ngân hàng (VietQR)</h4>
                </div>
                <div class="card-body">
                    <form action="{{ route('admin.settings.store') }}" method="post">
                        @csrf
                        <div class="mb-3">
                            ...
                            <input type="text" id="bank_name" name="bank_name" class="form-control" value="{{ setting('bank_name') }}" placeholder="Ví dụ: VietinBank">
                        </div>
                        <div class="mb-3">
                            <label for="bank_id" class="form-label">Mã ngân hàng (BIN)</label>
                            <input type="text" id="bank_id" name="bank_id" class="form-control" value="{{ setting('bank_id') }}" placeholder="Ví dụ: ICB">
                        </div>
                        <div class="mb-3">
                            <label for="bank_account_no" class="form-label">Số tài khoản</label>
                            <input type="text" id="bank_account_no" name="bank_account_no" class="form-control" value="{{ setting('bank_account_no') }}">
                        </div>
                        <div class="mb-3">
                            <label for="bank_account_name" class="form-label">Tên chủ tài khoản</label>
                            <input type="text" id="bank_account_name" name="bank_account_name" class="form-control" value="{{ setting('bank_account_name') }}">
                        </div>
                        <div class="mb-3">
                            <label for="bank_description" class="form-label">Cú pháp nạp tiền</label>
                            <input type="text" id="bank_description" name="bank_description" class="form-control" value="{{ setting('bank_description') }}" placeholder="Ví dụ: NAP">
                        </div>
                        <button type="submit" class="btn btn-primary">Lưu thay đổi</button>
                    </form>
                </div>
