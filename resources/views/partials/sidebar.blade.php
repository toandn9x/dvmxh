    ...
    <div id="scrollbar">
        <div class="container-fluid">
            ...
            <ul class="navbar-nav" id="navbar-nav">
                ...
                <li class="nav-item">
                    <a class="nav-link menu-link" href="#sidebarDeposit" data-bs-toggle="collapse" role="button"
                        aria-expanded="false" aria-controls="sidebarDeposit">
                        <i class="mdi mdi-cash-plus"></i> <span>Nạp tiền</span>
                    </a>
                    <div class="collapse menu-dropdown {{ Request::routeIs('deposit.*') ? 'show' : '' }}"
                        id="sidebarDeposit">
                        <ul class="nav nav-sm flex-column">
                            @can('charge-deposit')
                                <li class="nav-item">
                                    <a href="{{ route('deposit.thesieure') }}"
                                        class="nav-link {{ Request::routeIs('deposit.thesieure') ? 'active' : '' }}">Nạp
                                        Thesieure.com</a>
                                </li>
                            @endcan
                            <li class="nav-item">
                                <a href="{{ route('deposit.bank') }}"
                                    class="nav-link {{ Request::routeIs('deposit.bank') ? 'active' : '' }}">Chuyển
                                    khoản</a>
                            </li>
                            <li class="nav-item">
                                <a href="{{ route('deposit.charge') }}"
                                    class="nav-link {{ Request::routeIs('deposit.charge') ? 'active' : '' }}">Nạp thẻ
                                    cào</a>
                            </li>
                        </ul>
                    </div>
                ...
                <li class="nav-item">
                    <a class="nav-link menu-link {{ Request::routeIs('order.history') ? 'active' : '' }}"
                        href="{{ route('order.history') }}">
                        <i class="mdi mdi-cash-fast"></i> <span>Đơn đã đặt</span>
                    </a>
                </li>
                <li class="nav-item">
                    <a class="nav-link menu-link {{ Request::routeIs('user.tran-log') ? 'active' : '' }}"
                        href="{{ route('user.tran-log') }}">
                        <i class="mdi mdi-history"></i> <span>Lịch sử giao dịch</span>
                    </a>
                </li>
                <li class="nav-item">
                    <a class="nav-link menu-link" href="#sidebarTicket" data-bs-toggle="collapse" role="button"
                        aria-expanded="false" aria-controls="sidebarTicket">
                        <i class="mdi mdi-ticket-percent-outline"></i> <span>Hỗ trợ</span>
                    </a>
                    <div class="collapse menu-dropdown {{ Request::routeIs('ticket.*') ? 'show' : '' }}"
                        id="sidebarTicket">
                        <ul class="nav nav-sm flex-column">
                            <li class="nav-item">
                                <a href="{{ route('ticket.index') }}"
                                    class="nav-link {{ Request::routeIs('ticket.index') ? 'active' : '' }}">Danh sách hỗ
                                    trợ</a>
                            </li>
                            <li class="nav-item">
                                <a href="{{ route('ticket.create') }}"
                                    class="nav-link {{ Request::routeIs('ticket.create') ? 'active' : '' }}">Gửi yêu cầu
                                    hỗ trợ</a>
                            </li>
                        </ul>
                    </div>
                ...
                <li class="nav-item">
                    <a class="nav-link menu-link" href="#tools" data-bs-toggle="collapse" role="button"
                    ...
                    <div class="collapse menu-dropdown {{ Request::routeIs('tools.*') ? 'show' : '' }}" id="tools">
                        <ul class="nav nav-sm flex-column">
                            <li class="nav-item">
                                <a href="{{ route('tools.get-facebook-id') }}"
                                    class="nav-link {{ Request::routeIs('tools.get-facebook-id') ? 'active' : '' }}">Lấy
                                    ID Facebook</a>
                            </li>
                        </ul>
                    </div>
                ...
                <li class="nav-item">
                    <a class="nav-link menu-link {{ Request::routeIs('api.docs.*') ? 'active' : '' }}"
                        href="{{ route('api.docs.index') }}">
                        <i class="mdi mdi-api"></i> <span>Kết nối API</span>
                    </a>
                </li>
                @if($sidebarCategories->count() > 0)
                    ...
                    @foreach($sidebarCategories as $category)
                        <li class="nav-item">
                            <a class="nav-link menu-link" href="#{{ $category->slug }}" data-bs-toggle="collapse" role="button"
                                aria-expanded="false" aria-controls="{{ $category->slug }}">
                                <i class="{{ $category->icon }}"></i> <span>{{ $category->name }}</span>
                            </a>
                            <div class="collapse menu-dropdown {{ Request::routeIs('service.index') ? 'show' : '' }}"
                                id="{{ $category->slug }}">
                                <ul class="nav nav-sm flex-column">
                                    @foreach($category->services as $service)
                                        <li class="nav-item">
                                            <a href="{{ route('service.index', [$service->category->slug, $service->slug]) }}"
                ...
                @if(Auth::user()->isAdmin())
                    ...
                    <li class="nav-item">
                        <a class="nav-link menu-link {{ Request::routeIs('admin.money') ? 'active' : '' }}"
                            href="{{ route('admin.money') }}">
                            <i class="mdi mdi-cash"></i> <span>Cộng trừ tiền</span>
                        </a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link menu-link {{ Request::routeIs('admin.transactions') ? 'active' : '' }}"
                            href="{{ route('admin.transactions') }}">
                            <i class="mdi mdi-cash"></i> <span>Nhật ký giao dịch</span>
                        </a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link menu-link {{ Request::routeIs('admin.deposits.*') ? 'active' : '' }}"
                            href="{{ route('admin.deposits.index') }}">
                            <i class="mdi mdi-cash-check"></i> <span>Tiền đã nạp</span>
                        </a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link menu-link {{ Request::routeIs('admin.tickets.*') ? 'active' : '' }}"
                    ...
                    <li class="nav-item">
                        ...
                        <div class="collapse menu-dropdown {{ Request::routeIs('admin.settings.*') ? 'show' : '' }}"
                            id="sidebarSettings">
                            <ul class="nav nav-sm flex-column">
                                <li class="nav-item">
                                    <a href="{{ route('admin.settings.general') }}"
                                        class="nav-link {{ Request::routeIs('admin.settings.general') ? 'active' : '' }}">Cấu
                                        hình chung</a>
                                </li>
                                <li class="nav-item">
                                    <a href="{{ route('admin.settings.notifications') }}"
                                        class="nav-link {{ Request::routeIs('admin.settings.notifications') ? 'active' : '' }}">Thông
                                        báo</a>
                                </li>
                                <li class="nav-item">
                                    <a href="{{ route('admin.settings.deposit') }}"
                                        class="nav-link {{ Request::routeIs('admin.settings.deposit') ? 'active' : '' }}">Cấu
                                        hình nạp tiền</a>
                                </li>
                                <li class="nav-item">
                                    <a href="{{ route('admin.settings.levels') }}"
                                        class="nav-link {{ Request::routeIs('admin.settings.levels') ? 'active' : '' }}">Cấu
                                        hình Cấp độ & Thông báo</a>
                                </li>
                                <li class="nav-item">
                                    <a href="{{ route('admin.settings.smm') }}"
