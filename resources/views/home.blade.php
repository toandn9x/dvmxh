        ...
        <div class="col">
            <div class="h-100">
                ...
                <div class="row">
                    ...
                    <div class="col-xl-3 col-md-6">
                        <div class="card card-animate {{ Auth::user()->level === \App\Models\User::MEMBER ? 'bg-primary' : (Auth::user()->level === \App\Models\User::VIP ? 'bg-gradient-vip' : 'bg-gradient-collab') }}">
                            <div class="card-body">
                                <div class="d-flex align-items-center">
                                    <div class="flex-grow-1 overflow-hidden">
                                        <p class="text-uppercase fw-medium {{ Auth::user()->level === \App\Models\User::MEMBER ? 'text-white-50' : 'text-dark' }} text-truncate mb-0">Cấp độ tài khoản</p>
                                    </div>
                                </div>
                                <div class="d-flex align-items-end justify-content-between mt-4">
                                    <h4 class="fs-22 fw-semibold ff-secondary mb-4 {{ Auth::user()->level === \App\Models\User::MEMBER ? 'text-white' : 'text-dark' }}">
                                        {{ Auth::user()->levelName }}
                                    </h4>
                                    <div class="avatar-sm flex-shrink-0">
                                        <span class="avatar-title {{ Auth::user()->level === \App\Models\User::MEMBER ? 'bg-white bg-opacity-10 text-white' : 'bg-dark bg-opacity-10 text-dark' }} rounded fs-3">
                        ...
                        <div class="alert alert-primary border-0 rounded-top border-bottom border-primary border-3" role="alert">
                            <div class="d-flex align-items-center">
                                <div class="flex-shrink-0">
                                    <i class="ri-notification-3-line fs-24 text-primary"></i>
                                </div>
                                <div class="flex-grow-1 ms-3">
                                    <h5 class="alert-heading fs-16 mb-1">Thông báo hệ thống</h5>
                                    <div class="text-muted fs-14">
                                        {!! setting('home_notification', 'Chào mừng bạn đến với hệ thống dịch vụ mạng xã hội chuyên nghiệp.') !!}
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="col-12">
                        <div class="card">
                            <div class="card-header border-0">
                                <h4 class="card-title mb-0">Dịch vụ nổi bật</h4>
                            </div>
                            <div class="card-body">
                                <div class="row text-center">
                                    @foreach($sidebarCategories as $category)
                                        <div class="col-xl-2 col-md-4 col-6 mb-4">
                                            <a href="#{{ $category->slug }}" data-bs-toggle="collapse" class="text-reset d-block p-3 rounded bg-soft-primary card-animate h-100">
                                                <div class="avatar-sm mx-auto mb-3">
                                                    <span class="avatar-title bg-primary rounded-circle fs-2">
                                                        <i class="{{ $category->icon ?: 'ri-global-line' }}"></i>
                                                    </span>
                                                </div>
                                                <h5 class="fs-14 mb-0 text-truncate">{{ $category->name }}</h5>
                                            </a>
                                            <div class="collapse mt-2" id="{{ $category->slug }}">
                                                <ul class="list-group list-group-flush text-start shadow-sm rounded">
                                                    @foreach($category->services as $service)
                                                        <a href="{{ route('service.index', [$service->category->slug, $service->slug]) }}" class="list-group-item list-group-item-action py-2 px-3 fs-13">
                                                            <i class="ri-arrow-right-s-line align-middle me-1"></i> {{ $service->name }}
                                                        </a>
                                                    @endforeach
                                                </ul>
                                            </div>
